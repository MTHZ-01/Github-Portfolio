      ...
      <div className="max-w-7xl mx-auto px-6 z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div className={`${language === 'fa' ? 'md:order-2 text-right' : 'md:order-1 text-left'}`}>
            <h1 className="relative text-6xl md:text-8xl font-extrabold tracking-tight mb-4 leading-tight bg-gradient-to-b from-gray-100 via-gray-300 to-gray-500 bg-clip-text text-transparent">
              {language === 'fa' ? 'ساخت' : 'BUILDING'}
              <br />
              {language === 'fa' ? 'آینده' : 'THE FUTURE'}
            </h1>

            <p className="text-lg md:text-xl bg-gradient-to-b from-gray-200 to-gray-400 bg-clip-text text-transparent mb-8 max-w-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="px-6 py-3 bg-gray-600/8 text-gray-300 font-semibold rounded-full hover:bg-gray-500/10 transition text-base"
              >
          ...
          <div className={`${language === 'fa' ? 'md:order-1 md:justify-start' : 'md:order-2 md:justify-end'} flex justify-center` }>
            <div className="w-80 md:w-[420px] rounded-2xl overflow-hidden shadow-[0_20px_40px_rgba(100,100,100,0.02),inset_0_1px_1px_rgba(150,150,150,0.08)] border border-gray-600/15">
              <img src={beatifulPic} alt="Hero" className="w-full h-full object-cover block" />
            </div>
          </div>
        </div>
      </div>
